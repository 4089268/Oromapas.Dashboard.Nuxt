...
function mesAbrevToNum(mes: string): number {
  let mesIndex = mes.split(' ')[0] ?? '';
  return MONTH_MAP[mesIndex] ?? 0
}

const ALL_INGRESOS: Ingreso[] = [
...
export default defineEventHandler((event): Ingreso[] => {
  const q = getQuery(event)
  let list = [...ALL_INGRESOS]

  if (q.anio) {
    const anio = parseInt(String(q.anio))
    list = list.filter(i => i.anio === anio)
  }
